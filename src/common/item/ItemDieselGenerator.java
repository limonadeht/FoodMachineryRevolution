...
public class ItemDieselGenerator extends ItemBlock implements IFluidContainerItem{

	public static final String DISEL_TAG = "productTank";

	public ItemDieselGenerator(Block block){
		super(block);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean bool){
		DiselGenerator generator = (DiselGenerator)this.field_150939_a;
		...
		if(FoodMachineryRevolution.serverproxy.isShiftKeyDown()){
			list.add("If this Disel Generator is fill from 'BioDiesel'");
			list.add("to Generate Rf.");
			list.add("Generate: " + generator.energyGeneration + " RF");
		}else{
	...
	public static ItemStack createFilledTank(Fluid fluid) {
		final int tankCapacity = TileEntityDiselGenerator.getTankCapacity();
		...
		ItemStack item = new ItemStack(MaterialRegister.dieselGenerator);
		saveTank(item, tank);
		return item;
	}

	private static FluidTank readTank(ItemStack stack) {
		FluidTank tank = new FluidTank(TileEntityDiselGenerator.getTankCapacity());

		final NBTTagCompound itemTag = stack.getTagCompound();
		if (itemTag != null && itemTag.hasKey(DISEL_TAG)) {
			tank.readFromNBT(itemTag.getCompoundTag(DISEL_TAG));
			return tank;
		}
	...
	private static void saveTank(ItemStack container, FluidTank tank) {
		if (tank.getFluidAmount() > 0) {
			...
			itemTag.setTag(DISEL_TAG, tankTag);
		} else {
